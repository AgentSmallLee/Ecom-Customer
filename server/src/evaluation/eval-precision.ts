// server/src/evaluation/eval-precision.ts
...
// 单个模式的准确率结果（供 eval-all 汇总与回归对比使用）
export interface PrecisionSummary {
  mode: Mode;
  precisionByK: Record<number, number>;                      // K -> 总体准确率
  categoryPrecision: Record<string, Record<number, number>>; // 分类 -> K -> 准确率
}
...
export async function evaluateMode(evalSet: EvalItem[], mode: Mode, ks: number[]): Promise<PrecisionSummary> {
  console.log(`\n${'='.repeat(60)}`);
...
async function main() {
  ...
  if (mode === 'all') {
    ...
    const summaries: PrecisionSummary[] = [];
    ...
    for (const s of summaries) {
      console.log(`  ${s.mode.padEnd(10)}${ks.map(k => (s.precisionByK[k]?.toFixed(3) ?? '-').padEnd(12)).join('')}`);
    }
    console.log('');
  } else {
    // 只跑单个模式
